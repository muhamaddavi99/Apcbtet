...
interface Schedule {
  ...
  end_time: string;
  teacher_id: string;
  classes: { name: string; grade: string } | { name: string; grade: string }[] | null;
...
async function sendPush(endpoint: string, p256dh: string, auth: string, payload: object, vapidPub: string, vapidPriv: string): Promise<{ success: boolean; error?: string }> {
  try {
...
function getIndonesianDay(): string {
  ...
  return days[new Date(Date.now() + 7 * 60 * 60 * 1000).getUTCDay()];
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Starting schedule notification...');
    const supabase = createClient(supabaseUrl, supabaseServiceKey);
    const today = getIndonesianDay();
    console.log('Today:', today);

    if (today === 'Jumat') {
      return new Response(JSON.stringify({ success: true, message: 'Friday - skipped' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    const { data: schedules, error: schErr } = await supabase
      .from('schedules')
      .select('id, day, start_time, end_time, teacher_id, classes:class_id (name, grade), subjects:subject_id (name)')
      .eq('day', today);
    if (schErr) throw schErr;

    if (!schedules?.length) {
      return new Response(JSON.stringify({ success: true, message: 'No schedules' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    const byTeacher: Record<string, Schedule[]> = {};
    for (const s of schedules) {
      if (!byTeacher[s.teacher_id]) byTeacher[s.teacher_id] = [];
      byTeacher[s.teacher_id].push(s as Schedule);
    }
    ...
    if (!subs?.length) {
      return new Response(JSON.stringify({ success: true, message: 'No subs' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    const results: { userId: string; success: boolean; error?: string }[] = [];

    for (const sub of subs) {
      const teacherSchedules = byTeacher[sub.user_id];
      // Only send if teacher has schedule
      if (!teacherSchedules?.length) {
        console.log(`No schedule for ${sub.user_id}, skip`);
        continue;
      }

      teacherSchedules.sort((a, b) => a.start_time.localeCompare(b.start_time));
      const list = teacherSchedules.map(s => {
        const cls = Array.isArray(s.classes) ? s.classes[0] : s.classes;
        const subj = Array.isArray(s.subjects) ? s.subjects[0] : s.subjects;
        return `${s.start_time.substring(0, 5)} - ${cls?.name || '?'}: ${subj?.name || '?'}`;
      }).join('\n');

      const payload = {
        title: `📚 Jadwal Mengajar (${today})`,
        body: `${teacherSchedules.length} jadwal:\n${list}`,
        tag: 'daily-schedule',
        url: '/jadwal',
      };

      const result = await sendPush(sub.endpoint, sub.p256dh, sub.auth, payload, vapidPub, vapidPriv);
    ...
    return new Response(JSON.stringify({ success: true, sent: successCount, results }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error) {
